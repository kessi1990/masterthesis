...
training_steps = 5000  # 5000000
evaluation_start = 50  # 50000
...
if __name__ == '__main__':
    if model_type == 'cead':
        agent = models_v2.CEADNAgent(env.action_space.n, device, num_layers)
    else:
        agent = models_v2.DARQNAgent(env.action_space.n, device, num_layers)
    ...
    for step in range(1, training_steps + 1):
        ...
        if done:
            ...
            return_ = 0
        with torch.no_grad():
            action = agent.training_policy(state_seq)
        next_state, reward, done, info = env.step(action)
        return_ += reward
        next_state_seq.append(t.transform(next_state))
        ...
        if step % evaluation_start == 0 or step == training_steps:
            ...
            results = {'loss': losses, 'evaluation_returns': evaluation_returns, 'training_returns': training_returns, 'epsilons': epsilons}
