...
class DQNNew(Agent):
    ...
    def __init__(self, model_type, nr_actions, device, num_layers, hidden_size, alignment):
        ...
        self.device = device
        self.policy_net = models.CEADModel(nr_actions, device, num_layers, hidden_size, alignment).to(device)
        self.target_net = models.CEADModel(nr_actions, device, num_layers, hidden_size, alignment).to(device)
    ...
    def train(self):
        ...
        if self.gradient_clipping:
            for param in self.policy_net.parameters(): # only clamp lstm gradients (ltpwtl paper)
                param.grad.data.clamp_(min=self.gradient_threshold * -1, max=self.gradient_threshold)
